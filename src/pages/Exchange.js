...
function Exchange() {
	...
	const approve = async () => {
		...
		try {
			...
			var contract = new Contract(fromAddress.address, WBNB, signer);
			...
			await contract.approve(Router, amountIn.toString());
			setLoadding(false);
		} catch (err) {
			setLoadding(false);
			console.log(err);
	...
	const quateSwap = async (e) => {
		...
		try {
			...
			var fromAddressContract = new Contract(
				fromAddress.address,
				WBNB,
				signer
			);
			var fromAddressDecimals = await fromAddressContract.decimals();
			var allowance = await fromAddressContract.allowance(
				address,
				Router
			);
			...
			var toAddressContract = new Contract(
				toAddress.address,
				WBNB,
				signer
			);
			var toAddressDecimals = await toAddressContract.decimals();

			var contract = new Contract(Router, PANCAKE_ABI, signer);
			var WETH = await contract.WETH();
			...
			if (fromAddress.address === WETH) {
				var amountsOut = await contract.getAmountsOut(`${amountIn}`, [
					WETH,
					toAddress.address,
				]);
		...
		try {
			if (type === "From") {
				if (
					...
					var format = formatEther(result); // 29803630.997051883414242659
				...
				if (
					...
					var result = await provider.getBalance(address);
					var format = formatUnits(result); // 29803630.997051883414242659
							...
							<div className="checkout-login-step">
								...
								<div className="add-liquidity">
									<div className="balence">
										...
										<h6>
											balance:{" "}
											{parseFloat(fromBalance).toFixed(
												10
											)}
										</h6>
									</div>
								...
								<div className="add-liquidity">
									<div className="balence">
										...
										<h6>
											balance:{" "}
											{parseFloat(toBalance).toFixed(10)}
										</h6>
									</div>
									<div className="form-row">
										<div className="wizard-form-input col-md-6 col-sm-12">
